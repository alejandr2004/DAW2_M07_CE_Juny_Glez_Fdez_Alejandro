                ...
                @foreach($users as $user)
                    <tr data-id="{{ $user->id }}" class="hover:bg-gray-50">
                        ...
                        <td class="py-3 px-4 whitespace-nowrap flex gap-2">
                            @if(Auth::id() !== $user->id)
                                <form class="ajax-form" action="{{ route('admin.users.updateRole', $user) }}" method="POST" data-reset="false">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="role" value="{{ $user->role === 'admin' ? 'user' : 'admin' }}">
                                    <button type="submit" class="bg-yellow-500 hover:bg-yellow-600 text-white py-1 px-2 rounded text-sm">
